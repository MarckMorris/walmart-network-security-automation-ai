...
class NetworkLSTMPredictor:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized LSTM Predictor (sequence={sequence_length}, forecast={forecast_horizon})"
        )
    ...
    def train(
        ...
        logger.info(
            f"Training complete. Final val_loss: {train_metrics['final_val_loss']:.4f}"
        )
    ...
    def forecast(
        self, df: pd.DataFrame, feature_columns: List[str], steps_ahead: int = 6
    ) -> pd.DataFrame:
        """
        ...
        if len(df) < self.sequence_length:
            raise ValueError(
                f"Need at least {self.sequence_length} historical data points"
            )
        ...
        last_timestamp = (
            df.index[-1] if isinstance(df.index, pd.DatetimeIndex) else None
        )
        ...
        results = pd.DataFrame(
            {
                ...
                * 0.95,  # Simplified confidence intervals
        ...
        if last_timestamp:
            results["timestamp"] = pd.date_range(
                start=last_timestamp, periods=len(predictions) + 1, freq="H"
            )[1:]
